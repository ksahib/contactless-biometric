...
def _smooth_1d_series(values: np.ndarray, window: int) -> np.ndarray:
    """Smooth a 1D array with edge padding. Uses an odd window length."""
    arr = np.asarray(values, dtype=np.float32)
    if arr.size == 0:
        return arr
...
def _masked_gaussian_smooth(values: np.ndarray, mask: np.ndarray, sigma_x: float, sigma_y: float) -> np.ndarray:
    """Smooth a map inside a mask without bleeding zeros from the background."""
...
def _fill_small_map_holes_rowwise(values: np.ndarray, mask: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Fill row-wise holes between valid samples; does not extrapolate outside row support."""
    out = np.asarray(values, dtype=np.float32).copy()
    out_mask = mask.astype(bool).copy()
    h, _w = out.shape
    for y in range(h):
        cols = np.flatnonzero(out_mask[y] & np.isfinite(out[y]))
        if cols.size < 2:
            continue
        span = np.arange(cols[0], cols[-1] + 1)
        valid = out_mask[y, span] & np.isfinite(out[y, span])
        if np.count_nonzero(valid) < 2:
            continue
        valid_cols = span[valid]
        out[y, span] = np.interp(span, valid_cols, out[y, valid_cols]).astype(np.float32)
        out_mask[y, span] = True
    return out, out_mask.astype(np.uint8)


def _vertical_jitter_stats(source_x: np.ndarray, support: np.ndarray) -> dict[str, float]:
    valid = support.astype(bool) & np.isfinite(source_x)
    samples: list[np.ndarray] = []
    for y in range(source_x.shape[0] - 1):
        both = valid[y] & valid[y + 1]
        cols = np.flatnonzero(both)
        if cols.size == 0:
            continue
        samples.append(np.abs(source_x[y + 1, cols] - source_x[y, cols]).astype(np.float32))
    if not samples:
        return _finite_stats(np.asarray([], dtype=np.float32))
    return _finite_stats(np.concatenate(samples))


def _row_valid_bounds(mask_row: np.ndarray) -> tuple[int, int] | None:
    cols = np.flatnonzero(mask_row > 0)
    if cols.size == 0:
        return None
    return int(cols[0]), int(cols[-1])


def _interp_row_by_normalized_coordinate(
    values: np.ndarray,
    mask_row: np.ndarray,
    ...
    """Sample a source row into target columns using row-normalized x."""
    valid_cols = np.flatnonzero(mask_row > 0)
    if valid_cols.size < 2 or target_cols.size == 0:
        return np.full(target_cols.shape, np.nan, dtype=np.float32)
    src_left = float(valid_cols[0])
    src_right = float(valid_cols[-1])
    if src_right <= src_left or target_right <= target_left:
        return np.full(target_cols.shape, np.nan, dtype=np.float32)

    src_t = (valid_cols.astype(np.float32) - src_left) / max(src_right - src_left, 1e-6)
    target_t = (target_cols.astype(np.float32) - float(target_left)) / max(float(target_right - target_left), 1e-6)
    target_t = np.clip(target_t, 0.0, 1.0)
    return np.interp(target_t, src_t, values[valid_cols].astype(np.float32)).astype(np.float32)
...
def _fill_missing_output_rows(
    ...
    """Fill whole missing output rows by interpolating adjacent valid map rows.

    The black horizontal bands in v4 were not a texture problem; they were rows
    with no row_record because front/side normalized row mapping skips a few
    integer side rows. Row-wise hole filling cannot fix an entirely empty row,
    so this interpolates the source coordinate maps across those missing rows.
    """
    sx = np.asarray(source_x, dtype=np.float32).copy()
    sy = np.asarray(source_y, dtype=np.float32).copy()
    exp = np.asarray(expected, dtype=np.uint8).copy()
    valid_rows = np.flatnonzero(np.any(exp > 0, axis=1))
    if valid_rows.size < 2:
        return sx, sy, exp, 0

    filled = 0
    for start, stop in zip(valid_rows[:-1], valid_rows[1:]):
        gap = int(stop - start)
        if gap <= 1:
            continue
        start_bounds = _row_valid_bounds(exp[start])
        stop_bounds = _row_valid_bounds(exp[stop])
        if start_bounds is None or stop_bounds is None:
            continue
        s_left, s_right = start_bounds
        e_left, e_right = stop_bounds

        for y in range(int(start) + 1, int(stop)):
            alpha = float(y - start) / float(stop - start)
            left = (1.0 - alpha) * float(s_left) + alpha * float(e_left)
            right = (1.0 - alpha) * float(s_right) + alpha * float(e_right)
            c0 = max(0, int(round(left)))
            c1 = min(exp.shape[1] - 1, int(round(right)))
            if c1 <= c0:
                continue
            cols = np.arange(c0, c1 + 1, dtype=np.int32)
            sx_start = _interp_row_by_normalized_coordinate(sx[start], exp[start], cols, left, right)
            sx_stop = _interp_row_by_normalized_coordinate(sx[stop], exp[stop], cols, left, right)
            sy_start = _interp_row_by_normalized_coordinate(sy[start], exp[start], cols, left, right)
            sy_stop = _interp_row_by_normalized_coordinate(sy[stop], exp[stop], cols, left, right)

            sx[y, cols] = ((1.0 - alpha) * sx_start + alpha * sx_stop).astype(np.float32)
            sy[y, cols] = ((1.0 - alpha) * sy_start + alpha * sy_stop).astype(np.float32)
            exp[y, cols] = 1
            filled += 1

    return sx, sy, exp, filled
...
def _unwrap_from_depth_arc(
    ...
    output_width_scale: float = 1.0,
    gradient_clip: float | None = 3.0,
    row_param_smooth_window: int = 21,
    map_smooth_sigma_x: float = 2.0,
    map_smooth_sigma_y: float = 1.2,
) -> dict[str, np.ndarray | dict[str, Any]]:
    """Unwarp one side view from rendered depth with controlled output scale.

    The first version of this script used raw cumulative geodesic length as
    output pixel width. For side views that explodes the chart width because
    the rendered side depth has very steep slope near the silhouette. The
    result is a 3000-4000 px wide strip where most output pixels repeatedly
    sample almost the same input x coordinate.

    This version still uses depth-derived arc length to redistribute pixels
    nonlinearly inside each row, but it normalizes that row's arc coordinate
    back to a controlled width derived from the observed support-row width.

    In other words:
        source row columns -> depth arc coordinate -> normalized to row width

    This keeps the output at the side fingerprint scale while preserving the
    local perspective correction implied by the rendered depth.
    """
    h, w = image.shape
    output_width_scale = float(max(output_width_scale, 0.1))
    support_bool = support.astype(bool) & np.isfinite(depth)
    row_records: list[dict[str, Any]] = []
    max_width = 1
    raw_arc_spans: list[float] = []
    row_widths: list[int] = []
    ...
    for y in range(h):
        ...
        dz = np.diff(z)
        # Large silhouette depth jumps should not dominate the chart scale.
        # They are visibility/edge discontinuities more than useful ridge-scale
        # perspective correction. Clipping preserves ordering and local
        # nonlinearity without allowing one edge to make the output thousands
        # of pixels wide.
        if gradient_clip is not None and math.isfinite(float(gradient_clip)) and float(gradient_clip) > 0:
            dz = np.clip(dz, -float(gradient_clip), float(gradient_clip)).astype(np.float32)

        ds = np.sqrt(1.0 + dz * dz).astype(np.float32)
        s = np.concatenate([[0.0], np.cumsum(ds)]).astype(np.float32)
        if not np.isfinite(s).all() or float(s[-1] - s[0]) <= 1e-6:
            continue

        source_cols = cols.astype(np.float32)
        if reverse_x:
            # Reverse orientation by reversing both surface coordinate and
            # source column samples, then re-normalize s to increasing order.
            source_cols = source_cols[::-1].copy()
            s = (float(s[-1]) - s)[::-1].astype(np.float32)
            s = s - float(s[0])

        # Controlled row width. Use observed rendered support width, not raw
        # geodesic length. This is the main fix for noisy/streaky output.
        observed_row_width = int(cols[-1] - cols[0] + 1)
        out_row_width = max(2, int(round(observed_row_width * output_width_scale)))
        max_width = max(max_width, out_row_width)
        raw_arc_spans.append(float(s[-1] - s[0]))
        row_widths.append(out_row_width)
        row_records.append(
            {
                "y": int(y),
                "cols": source_cols.astype(np.float32),
                "s": s.astype(np.float32),
                "s_min": float(s[0]),
                "s_max": float(s[-1]),
                "width": int(out_row_width),
                "observed_row_width": int(observed_row_width),
                "raw_arc_span": float(s[-1] - s[0]),
            }
        )
    ...
    # Keep the valid side-visible chart from v2, but remove row-to-row scale
    # jitter before any remap is built. This targets the actual ridge-noise
    # cause: neighboring rows previously used independently fluctuating widths.
    row_records = sorted(row_records, key=lambda r: int(r["y"]))
    widths_raw = np.asarray([float(r["width"]) for r in row_records], dtype=np.float32)
    widths_smooth = _smooth_1d_series(widths_raw, row_param_smooth_window)
    for rec, smooth_width in zip(row_records, widths_smooth):
        observed = int(rec["observed_row_width"])
        lower = max(2, int(round(0.85 * observed)))
        upper = max(lower, int(round(1.15 * observed)))
        rec["width"] = int(np.clip(round(float(smooth_width)), lower, upper))
    max_width = max(int(r["width"]) for r in row_records)
    ...
    for rec in row_records:
        ...
        width = int(rec["width"])
        if s.size < 2 or cols.size < 2:
            continue

        # Center each row in the output canvas to preserve finger silhouette.
        offset = (out_w - width) // 2
        # Normalize output x to the row's arc range instead of using arc length
        # directly as the number of output pixels.
        target_s = np.linspace(float(s[0]), float(s[-1]), width, dtype=np.float32)
    ...
    # Fill whole missing rows before smoothing. These rows are the black bands
    # visible in v4 output/masks when normalized front->side row mapping skipped
    # a few integer rows.
    source_x, source_y, expected, filled_missing_rows = _fill_missing_output_rows(source_x, source_y, expected)

    # Smooth the source coordinate maps as a 2D field. This treats the main
    # problem directly: source-coordinate jitter between adjacent rows. The
    # texture itself is not blurred; only the remap field is regularized.
    map_mask = expected.astype(bool) & np.isfinite(source_x) & np.isfinite(source_y)
    source_x, map_mask = _fill_small_map_holes_rowwise(source_x, map_mask.astype(np.uint8))
    source_y, _ = _fill_small_map_holes_rowwise(source_y, map_mask.astype(np.uint8))
    map_mask = map_mask.astype(bool) & np.isfinite(source_x) & np.isfinite(source_y)
    if map_smooth_sigma_x > 0 or map_smooth_sigma_y > 0:
        source_x = _masked_gaussian_smooth(source_x, map_mask, map_smooth_sigma_x, map_smooth_sigma_y)
        source_y = _masked_gaussian_smooth(source_y, map_mask, map_smooth_sigma_x, map_smooth_sigma_y)
    ...
    # Do not re-mask by remapping the sparse rendered-depth support with
    # nearest-neighbor. The rows filled above intentionally interpolate between
    # adjacent valid rendered rows; remapping support_bool can still evaluate to
    # false on those interpolated rows and recreate the black bands. The map
    # validity is now defined by the filled/smoothed expected chart plus bounds.
    sampled_support = inside.copy()
    unwrapped[~sampled_support] = 0

    source_x_step = _source_x_step_stats(source_x, sampled_support)
    median_row_width = float(np.median(np.asarray(row_widths, dtype=np.float32))) if row_widths else 0.0
    median_raw_arc = float(np.median(np.asarray(raw_arc_spans, dtype=np.float32))) if raw_arc_spans else 0.0
    report = {
        ...
        "source_x_step_stats": source_x_step,
        "vertical_jitter_stats": _vertical_jitter_stats(source_x, sampled_support),
        "reverse_x": bool(reverse_x),
        "output_width_scale": float(output_width_scale),
        "gradient_clip": None if gradient_clip is None else float(gradient_clip),
        "row_param_smooth_window": int(row_param_smooth_window),
        "map_smooth_sigma_x": float(map_smooth_sigma_x),
        "map_smooth_sigma_y": float(map_smooth_sigma_y),
        "filled_missing_output_rows": int(filled_missing_rows),
        "unwrap_mask_source": "filled_expected_chart_and_bounds",
        "median_output_row_width": median_row_width,
        "median_raw_arc_span_before_normalization": median_raw_arc,
        "raw_arc_to_output_width_ratio_median": float(median_raw_arc / max(median_row_width, 1.0)),
        "scale_gate_passed": bool(source_x_step.get("median", 0.0) >= 0.25),
        "notes": [
            "Output width is normalized to observed side support width, not raw geodesic arc length.",
            "Depth gradients redistribute x samples, missing output rows are interpolated, then row parameters and 2D source maps are smoothed for row coherence.",
        ],
    }
    return {
...
def run(
    ...
    report: dict[str, Any] = {
        ...
        "method": "Algorithm 1 canonical 3D surface -> fixed-view projection with z-buffer -> per-view depth maps -> normalized depth-arc side unwrap with row-coherent map smoothing",
        ...
        "notes": [
            ...
            "Depth unwrapping normalizes each row's arc coordinate back to observed side-row width to avoid raw geodesic width explosion.",
        ],
        "unwrap_width_scale": float(unwrap_width_scale),
        "unwrap_gradient_clip": None if unwrap_gradient_clip is None else float(unwrap_gradient_clip),
        "row_param_smooth_window": int(row_param_smooth_window),
        "map_smooth_sigma_x": float(map_smooth_sigma_x),
        "map_smooth_sigma_y": float(map_smooth_sigma_y),
        "roles": {},
    ...
    for role, reverse_x in (("left", reverse_left_unwrap_x), ("right", reverse_right_unwrap_x)):
        unwrapped[role] = _unwrap_from_depth_arc(
            ...
            rendered[role]["depth"],
            reverse_x=reverse_x,
            output_width_scale=unwrap_width_scale,
            gradient_clip=unwrap_gradient_clip,
...
def main() -> int:
    ...
    parser.add_argument("--unwrap-width-scale", type=float, default=1.0, help="Scale factor applied to each observed side support row width when choosing unwrapped row width. Keep near 1.0.")
    parser.add_argument("--unwrap-gradient-clip", type=float, default=3.0, help="Clip per-pixel depth differences before arc integration. Use <=0 to disable.")
    parser.add_argument("--row-param-smooth-window", type=int, default=21, help="Odd moving-average window for row width smoothing.")
    parser.add_argument("--map-smooth-sigma-x", type=float, default=2.0, help="Gaussian sigma in x for source-coordinate map smoothing.")
    parser.add_argument("--map-smooth-sigma-y", type=float, default=1.2, help="Gaussian sigma in y for source-coordinate map smoothing.")
