#!/usr/bin/env python
"""Algorithm-1 side-view ground-truth depth rendering and canonical-chart unwrapping.

Long-term fix implemented here:
    front/left/right masks
      -> Algorithm 1 canonical 3D ellipse surface
      -> fixed-view projection with z-buffer visibility
      -> per-view rendered depth maps and canonical surface-coordinate maps
      -> side-view unwrapping by inverting a *global canonical surface chart*

This addresses the main problem in the previous scripts: they built the side
unwrap from independent per-row remaps (first directly from stitched side
curves, then from per-row depth-arc redistribution). Those methods produce
row-to-row jitter in source_x, which shows up as noisy / zig-zag ridges.

This version instead uses the rendered canonical surface coordinates as the
source of truth:
  - Each visible side-view pixel already corresponds to a point on the
    canonical 3D model.
  - We store that point's canonical cross-section coordinate x/a and canonical
    front-row coordinate y_front.
  - The side unwrap is built by globally inverting that 2D surface->image map.
  - The resulting source_x/source_y maps are smoothed as a 2D field, not
    reconstructed independently row by row.
...
SQRT2 = math.sqrt(2.0)
DEFAULT_VIEW_ANGLES_DEG = {"front": 0.0, "left": -45.0, "right": 45.0}
...
def _render_view_depth(
    ...
    canonical_y = np.full((h, w), np.nan, dtype=np.float32)
    a_map = np.full((h, w), np.nan, dtype=np.float32)
    b_map = np.full((h, w), np.nan, dtype=np.float32)
    cz_map = np.full((h, w), np.nan, dtype=np.float32)
    ...
    for y_front in front_rows:
        ...
        canonical_y[y_target, replace_cols] = float(y_front)
        branch_map[y_target, replace_cols] = br_interp[replace]
        a_map[y_target, replace_cols] = float(params.a)
        b_map[y_target, replace_cols] = float(params.b)
        cz_map[y_target, replace_cols] = float(params.cz)
    ...
    return {
        ...
        "canonical_y": canonical_y.astype(np.float32),
        "a_map": a_map.astype(np.float32),
        "b_map": b_map.astype(np.float32),
        "cz_map": cz_map.astype(np.float32),
...
def _masked_gaussian(field: np.ndarray, mask: np.ndarray, sigma_x: float, sigma_y: float) -> np.ndarray:
    mask_f = mask.astype(np.float32)
    weighted = np.where(mask, field, 0.0).astype(np.float32)
    blur_num = cv2.GaussianBlur(weighted, (0, 0), sigmaX=sigma_x, sigmaY=sigma_y)
    blur_den = cv2.GaussianBlur(mask_f, (0, 0), sigmaX=sigma_x, sigmaY=sigma_y)
    out = np.divide(blur_num, np.maximum(blur_den, 1e-6)).astype(np.float32)
    return out


def _fill_masked_holes_rowwise(field: np.ndarray, mask: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    for y in range(h):
        ...
        valid = m[y, span] & np.isfinite(out[y, span])
        if np.count_nonzero(valid) < 2:
            continue
        valid_cols = span[valid]
        out[y, span] = np.interp(span, valid_cols, out[y, valid_cols]).astype(np.float32)
        m[y, span] = True
    return out, m.astype(np.uint8)


def _fill_masked_holes_colwise(field: np.ndarray, mask: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    out = field.copy().astype(np.float32)
    m = mask.copy().astype(bool)
    h, w = out.shape
    for x in range(w):
        ...
        if rows.size < 2:
            continue
        span = np.arange(rows[0], rows[-1] + 1)
        valid = m[span, x] & np.isfinite(out[span, x])
        if np.count_nonzero(valid) < 2:
            continue
        valid_rows = span[valid]
        out[span, x] = np.interp(span, valid_rows, out[valid_rows, x]).astype(np.float32)
        m[span, x] = True
    return out, m.astype(np.uint8)
...
def _vertical_jitter_stats(source_x: np.ndarray, support: np.ndarray) -> dict[str, float]:
    ...
    for y in range(source_x.shape[0] - 1):
        ...
        if cols.size == 0:
            continue
        vals.append(np.abs(source_x[y + 1, cols] - source_x[y, cols]).astype(np.float32))
    if not vals:
        return _finite_stats(np.asarray([], dtype=np.float32))
    return _finite_stats(np.concatenate(vals))


def _unwrap_from_canonical_chart(
    image: np.ndarray,
    support: np.ndarray,
    ...
    smooth_sigma_x: float = 4.0,
    smooth_sigma_y: float = 2.0,
) -> dict[str, np.ndarray | dict[str, Any]]:
    """Build side unwrap by globally inverting the canonical surface chart.

    Key difference from the old approach:
      - No independent per-row depth-arc remapping.
      - Each side-view support pixel contributes to a single 2D chart coordinate
        based on canonical model coordinates (x/a, y_front).
      - We invert that chart->image relation with bilinear splatting and then
        smooth the *source coordinate field* as a 2D surface map.
    """
    h, w = image.shape
    valid = (
        support.astype(bool)
        & np.isfinite(canonical_x)
        & np.isfinite(canonical_y)
        & np.isfinite(a_map)
        & (a_map > 1e-6)
    )
    if not np.any(valid):
        raise RuntimeError("No valid canonical chart coordinates available for unwrap")

    y_vals = canonical_y[valid].astype(np.float32)
    y0 = int(math.floor(float(np.min(y_vals))))
    y1 = int(math.ceil(float(np.max(y_vals))))
    out_h = max(1, y1 - y0 + 1)
    ...
    for y in range(h):
        cols = np.flatnonzero(valid[y])
        if cols.size >= 2:
            support_widths.append(int(cols[-1] - cols[0] + 1))
    if not support_widths:
        raise RuntimeError("No valid support rows available for canonical-chart unwrap")
    base_width = int(round(np.percentile(np.asarray(support_widths, dtype=np.float32), 99)))
    out_w = max(2, int(round(base_width * max(output_width_scale, 0.1))))

    x_norm = np.clip(canonical_x[valid] / np.maximum(a_map[valid], 1e-6), -1.0, 1.0).astype(np.float32)
    u = 0.5 * (x_norm + 1.0)
    if reverse_x:
        u = 1.0 - u
    v = (canonical_y[valid].astype(np.float32) - float(y0)).astype(np.float32)
    j = np.clip(u * float(out_w - 1), 0.0, float(out_w - 1))
    i = np.clip(v, 0.0, float(out_h - 1))

    src_y_idx, src_x_idx = np.nonzero(valid)
    src_x_vals = src_x_idx.astype(np.float32)
    src_y_vals = src_y_idx.astype(np.float32)

    accum_x = np.zeros((out_h, out_w), dtype=np.float32)
    accum_y = np.zeros((out_h, out_w), dtype=np.float32)
    accum_w = np.zeros((out_h, out_w), dtype=np.float32)

    i0 = np.floor(i).astype(np.int32)
    j0 = np.floor(j).astype(np.int32)
    i1 = np.clip(i0 + 1, 0, out_h - 1)
    j1 = np.clip(j0 + 1, 0, out_w - 1)
    ...
    for ww, rr, cc in weights:
        np.add.at(accum_x, (rr, cc), ww * src_x_vals)
        np.add.at(accum_y, (rr, cc), ww * src_y_vals)
        np.add.at(accum_w, (rr, cc), ww)

    chart_mask = accum_w > 1e-6
    source_x = np.divide(accum_x, np.maximum(accum_w, 1e-6)).astype(np.float32)
    source_y = np.divide(accum_y, np.maximum(accum_w, 1e-6)).astype(np.float32)
    source_x[~chart_mask] = np.nan
    source_y[~chart_mask] = np.nan

    source_x, chart_mask_u8 = _fill_masked_holes_rowwise(source_x, chart_mask.astype(np.uint8))
    source_y, chart_mask_u8 = _fill_masked_holes_rowwise(source_y, chart_mask_u8)
    source_x, chart_mask_u8 = _fill_masked_holes_colwise(source_x, chart_mask_u8)
    source_y, chart_mask_u8 = _fill_masked_holes_colwise(source_y, chart_mask_u8)
    chart_mask = chart_mask_u8.astype(bool)

    # Smooth the *2D source-coordinate field* rather than doing per-row remaps.
    source_x_smooth = _masked_gaussian(source_x, chart_mask, sigma_x=smooth_sigma_x, sigma_y=smooth_sigma_y)
    source_y_smooth = _masked_gaussian(source_y, chart_mask, sigma_x=smooth_sigma_x, sigma_y=smooth_sigma_y)
    source_x[chart_mask] = source_x_smooth[chart_mask]
    source_y[chart_mask] = source_y_smooth[chart_mask]
    ...
    sampled_support = cv2.remap((support.astype(np.uint8) * 255), remap_x, remap_y, interpolation=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=0) > 0
    sampled_support &= inside
    unwrapped[~sampled_support] = 0

    support_rows = np.flatnonzero(np.any(sampled_support, axis=1))
    median_output_row_width = 0.0
    if support_rows.size > 0:
        widths = []
        for y in support_rows:
            cols = np.flatnonzero(sampled_support[y])
            if cols.size:
                widths.append(float(cols[-1] - cols[0] + 1))
        if widths:
            median_output_row_width = float(np.median(np.asarray(widths, dtype=np.float32)))

    report = {
        "output_shape_hw": [int(out_h), int(out_w)],
        "support_pixels": int(np.count_nonzero(sampled_support)),
        "expected_pixels": int(np.count_nonzero(chart_mask)),
        "support_over_expected": float(np.count_nonzero(sampled_support) / max(np.count_nonzero(chart_mask), 1)),
        ...
        "smooth_sigma_y": float(smooth_sigma_y),
        "median_output_row_width": float(median_output_row_width),
        "notes": [
            "Unwrap is built from the global canonical surface chart (x/a, y_front), not per-row depth-arc redistribution.",
            "Source coordinate maps are smoothed as a 2D field to preserve row-to-row coherence.",
        ],
    }
    return {
        "unwrapped": unwrapped.astype(np.uint8),
        "mask": sampled_support.astype(np.uint8),
        "expected": chart_mask.astype(np.uint8),
...
def _write_contact_sheet(out_dir: Path, views: dict[str, ViewData], rendered: dict[str, dict[str, Any]], unwrapped: dict[str, dict[str, Any]]) -> Path:
    ...
    for role in ("left", "right"):
        ...
        panels = [
            ...
            _draw_label(_fit_panel(unwrapped[role]["unwrapped"]), f"{role} canonical unwrap"),
...
def run(
    ...
    report: dict[str, Any] = {
        ...
        "method": "Algorithm 1 canonical 3D surface -> fixed-view projection with z-buffer -> per-view depth maps -> global canonical-chart side unwrap",
        ...
        "notes": [
            ...
            "No stitched side branch, per-row dominant segment, or per-row depth-arc redistribution is used as the unwrap source of truth.",
            "Side unwrap is obtained by globally inverting the rendered canonical surface chart (x/a, y_front).",
            "Source coordinate maps are smoothed as a 2D field to improve row coherence and reduce ridge jitter.",
        ],
    ...
    for role in ("front", "left", "right"):
        ...
        np.savez_compressed(
            ...
            canonical_y=rendered[role]["canonical_y"],
            a_map=rendered[role]["a_map"],
            b_map=rendered[role]["b_map"],
            cz_map=rendered[role]["cz_map"],
        )
    ...
    for role, reverse_x in (("left", reverse_left_unwrap_x), ("right", reverse_right_unwrap_x)):
        unwrapped[role] = _unwrap_from_canonical_chart(
            views[role].image,
            rendered[role]["support"],
            rendered[role]["canonical_x"],
            rendered[role]["canonical_y"],
            rendered[role]["a_map"],
            reverse_x=reverse_x,
...
def main() -> int:
    ...
    parser.add_argument("--left-angle", type=float, default=DEFAULT_VIEW_ANGLES_DEG["left"])
    parser.add_argument("--right-angle", type=float, default=DEFAULT_VIEW_ANGLES_DEG["right"])
    ...
    parser.add_argument("--smooth-sigma-x", type=float, default=4.0)
    parser.add_argument("--smooth-sigma-y", type=float, default=2.0)
